...
function printUppercase(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const uppercaseElement = array[i].toUpperCase();
    console.log(uppercaseElement); // Виводимо перетворений елемент на консоль
  }
...
function printStrings(array) {
  const arrayAsString = array.toString(); // Перетворюємо масив в рядок
  const repeatedString = arrayAsString + arrayAsString; // Повторюємо рядок двічі
  console.log(repeatedString); // Виводимо повторений рядок на консоль
...
function concatenateStrings(array, subString) {
  const arrayAsString = array.toString(); // Перетворюємо масив в рядок за допомогою toString()
  const containsSubstring = arrayAsString.includes(subString); // Перевіряємо чи містить рядок підрядок
...
function addTenToEach(array) {
  const resultArray = []; // Ініціалізуємо новий масив для зберігання результатів

  for (let index in array) {
    const newValue = array[index] + 10; // Додаємо 10 до поточного елемента
    resultArray.push(newValue); // Додаємо результат в новий масив
  }

  console.log(resultArray); // Виводимо новий масив на консоль
...
function swapMinMax(array) {
  let max = array[0]; // Ініціалізуємо змінну max першим елементом масиву
  let min = array[0]; // Ініціалізуємо змінну min першим елементом масиву
  let maxIndex = 0; // Ініціалізуємо змінну maxIndex зі значенням 0
  let minIndex = 0; // Ініціалізуємо змінну minIndex зі значенням 0
  ...
  [array[maxIndex], array[minIndex]] = [array[minIndex], array[maxIndex]]; // Обмін місцями мінімального та максимального значення

  return array; // Повертаємо масив з обміненими значеннями
...
function getEvenNumbers(arr) {
  const evenNumbers = []; // Створюємо пустий масив для зберігання парних чисел

  for (let num of arr) {
    if (num % 2 === 0) {
      evenNumbers.push(num); // Додаємо парне число до масиву результату
    }
  }
...
function multiplyByIndex(arr) {
  const resultArray = []; // Створюємо пустий масив для зберігання результату

  for (let i = 0; i < arr.length; i++) {
    resultArray.push(arr[i] * i); // Помножуємо число на індекс та додаємо до масиву результату
  }

  return resultArray; // Повертаємо масив з помноженими значеннями
}
...
function replaceNumbers(arr) {
  ...
  for (let num of arr) {
    if (num > 10) {
      resultArray.push("Greater than 10"); // Додаємо рядок "Greater than 10"
    } else {
      resultArray.push("Less than or equal to 10"); // Додаємо рядок "Less than or equal to 10"
    }
  }

  return resultArray; // Повертаємо масив результату
